...
async def startup():
    ...
    try:
        _opening_audio = await generate_tts_async(OUTBOUND_OPENING, model="bulbul:v3", speaker="simran")
...
async def shutdown():import logging
import os
import asyncio
import time
from dotenv import load_dotenv
...
async def _run_pipeline(messages: list[dict], call_sid: str, user_input: str) -> str:
    """
    Groq streaming → WS TTS streaming.

    Both run concurrently: TTS starts receiving/rendering audio at the
    first sentence boundary, while LLM is still generating the rest.
    This is the primary latency win — no waiting for full LLM output.
    """
    t0 = time.perf_counter()
    ...
    groq_stream = await groq_client.chat.completions.create(
        model="llama-3.1-8b-instant",
        messages=messages,
        max_tokens=55,          # 1 short Hindi sentence — keep audio duration short
        temperature=0.3,
        stream=True,
    )
    log.info("⏱  Groq stream open: %.3fs", time.perf_counter() - t0)

    # llm_to_tts_stream runs _send_tokens and _recv_audio concurrently inside
    audio_filename, ai_reply = await llm_to_tts_stream(groq_stream)
    ...
    log.info("⏱  _run_pipeline total: %.3fs", time.perf_counter() - t0)
    return audio_filename
...
async def _handle_voice(form_like) -> Response:
    ...
    try:
        ...
        messages = [{"role": "system", "content": SYSTEM_PROMPT}]
        # Keep only last 10 history messages (5 turns) — less context = faster LLM TTFT
        ...
        # speechTimeout="1" — stop listening sooner after user pauses (saves ~1s per turn)
        # timeout="6" — reduced from 8, user usually responds quickly
        twiml = f"""<?xml version="1.0" encoding="UTF-8"?>
<Response>
    <Play>{audio_url}</Play>
    <Gather input="speech" action="{record_action}" method="POST"
            speechTimeout="1" timeout="6" actionOnEmptyResult="true"
            language="hi-IN" enhanced="true" />
</Response>"""
        log.info("⏱  _handle_voice total: %.3fs", time.perf_counter() - t0)
        return Response(content=twiml, media_type="text/xml")
...
<Response>
    <Say language="hi-IN">Technical issue. Please try again.</Say>
    <Gather input="speech" action="{ngrok_url}/voice" method="POST"
            speechTimeout="1" timeout="6" actionOnEmptyResult="true" language="hi-IN" />
</Response>"""
        return Response(content=twiml, media_type="text/xml")
